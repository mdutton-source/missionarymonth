...
exports.handler = async function(event, context) {
  ...
  try {
    ...
    if (path.startsWith("/user/") && method === "GET") {
      ...
      if (missionaries.length === 0) return { statusCode: 404, headers, body: JSON.stringify({ error: "Not found" }) };
      ...
      return { statusCode: 200, headers, body: JSON.stringify({ name: m.name, zone: m.zone, days, weeks, bonus: bonusRows.length > 0 ? bonusRows[0].bonuses : {} }) };
    }
    ...
    if (path === "/submit" && method === "POST") {
      ...
      if (dateKey && habits) await sql`INSERT INTO daily_entries (missionary_name, date_key, habits) VALUES (${name}, ${dateKey}, ${JSON.stringify(habits)}) ON CONFLICT (missionary_name, date_key) DO UPDATE SET habits = ${JSON.stringify(habits)}, submitted_at = NOW()`;
      if (weekKey && challenges) await sql`INSERT INTO weekly_entries (missionary_name, week_key, challenges) VALUES (${name}, ${weekKey}, ${JSON.stringify(challenges)}) ON CONFLICT (missionary_name, week_key) DOUPDATE SET challenges = ${JSON.stringify(challenges)}, submitted_at = NOW()`;
      if (bonus) await sql`INSERT INTO bonus_entries (missionary_name, bonuses) VALUES (${name}, ${JSON.stringify(bonus)}) ON CONFLICT (missionary_name) DO UPDATE SET bonuses = ${JSON.stringify(bonus)}, submitted_at = NOW()`;
